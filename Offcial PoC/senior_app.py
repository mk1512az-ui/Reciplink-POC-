...
# -------------------------
class SeniorApp:
    def __init__(self, root, senior_id=1):
        self.root = root
        self.root.title("Senior Dashboard")
        self.root.geometry("700x500")
        self.senior_id = senior_id  # Replace with logged-in user's ID later
    ...
    def load_services(self):
        ...
        for row in c.execute("SELECT id, service_name, provider_id, provider_overall_rating, payment_amount FROM services"):
            provider_name = c.execute("SELECT name FROM providers WHERE id=?", (row[2],)).fetchone()
            self.service_tree.insert("", "end", values=(row[0], row[1], provider_name[0] if provider_name else "Unknown", row[3], row[4]))
    ...
    def load_bills(self):
        ...
        for row in c.execute("SELECT bill_id, provider_id, amount, status FROM bills WHERE senior_id=?", (self.senior_id,)):
            pname = c.execute("SELECT name FROM providers WHERE id=?", (row[1],)).fetchone()
            self.bill_tree.insert("", "end", values=(row[0], pname[0] if pname else "Unknown", row[2], row[3]))
